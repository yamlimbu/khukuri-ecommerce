...
- [ ] Inspect `apps/server/src/vendure-config.ts` and `apps/server/src/Local_vendure-config.ts` for current URL/email and dbConnectionOptions behavior.
- [ ] Build a single source of truth for Frontend/Backend base URLs depending on `APP_ENV === 'local'`.
  - [ ] Local: frontend `http://localhost:3001/`, backend `http://localhost:3000/`
  - [ ] Else: frontend `https://khukuri1-store.vercel.app/`, backend `https://khukuri-ecommerce.onrender.com/`
